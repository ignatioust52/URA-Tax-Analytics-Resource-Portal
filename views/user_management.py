...
Three tabs:
  ...
  3. Create Account    — admin-direct bypass of the registration queue
...
def render_user_management():
    ...
    with tab_audit:
        ...
        st.markdown(
            '<div class="section-title" style="border-bottom: 1px solid #E3E8EF; '
            'padding-bottom: 0.4rem; font-size: 1rem; margin-bottom: 0.8rem;">'
            "Activity Audit Log</div>",
            unsafe_allow_html=True,
        )
        st.caption("Most recent 200 platform events, newest first.")
    ...
    # ── TAB 6: Announcements ─────────────────────────────────────────────
    with tab_announce:
        st.markdown("<div class=\"section-title\">Manage Announcements</div>", unsafe_allow_html=True)
        st.caption("Broadcast messages to all users or specific departments.")
            ...
            for _, ann in ann_df.iterrows():
                with st.container(border=True):
                    ...
                    with col2:
                        ...
                        st.write(status)
                        if st.button("Delete", key=f"del_ann_{ann['announcement_id']}", icon=":material/delete:"):
                            announcements_delete(ann["announcement_id"])
                            st.rerun()
                        if st.button("Toggle Status", key=f"tgl_ann_{ann['announcement_id']}", icon=":material/toggle_on:"):
                            announcements_update(
