...
def render_analytics():
    ...
    with col_chart1:
        with st.container(border=True):
            ...
            st.line_chart(daily_views, x="date", y="views", use_container_width=True)
    
    with col_chart2:
        with st.container(border=True):
            st.markdown('<div class="ui-card-title">Top 10 Most Popular Resources</div>', unsafe_allow_html=True)
            popular = views_df.groupby("resource_id").size().reset_index(name="views").sort_values("views", ascending=False).head(10)
            st.bar_chart(popular, x="resource_id", y="views", use_container_width=True)
